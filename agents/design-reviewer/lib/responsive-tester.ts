...
export interface ResponsiveIssue {
  type: 'horizontal-scroll' | 'touch-target' | 'text-readability' | 'navigation' | 'content-overflow';
...
export class ResponsiveTester {
  ...
  async testViewport(viewport: { name: string; width: number; height: number }): Promise<ViewportTestResult> {
  ...
  private async checkTouchTargets(viewportName: string): Promise<ResponsiveIssue[]> {
    const touchTargetIssues = await this.page.evaluate((viewport) => {
      const interactive = document.querySelectorAll('button, a, input[type="button"], input[type="submit"], select, [role="button"]');
      const issues: any[] = [];
  ...
  private async checkTextReadability(viewportName: string): Promise<ResponsiveIssue[]> {
  ...
  private async checkNavigation(viewportName: string): Promise<ResponsiveIssue[]> {
    const navIssues = await this.page.evaluate((viewport) => {
      ...
      if (nav) {
        ...
        if (style.display === 'none' && viewport === 'Mobile') {
          ...
          const mobileToggle = document.querySelector('[aria-label*="menu"], [aria-label*="toggle"], .menu-toggle, .hamburger');
          if (!mobileToggle) {
            issues.push({
              type: 'navigation',
              severity: 'critical',
              viewport: viewport,
              message: 'Navigation hidden on mobile without accessible toggle button'
            });
          }
        }
      }
  ...
  private async checkContentOverflow(viewportName: string): Promise<ResponsiveIssue[]> {
    const overflowIssues = await this.page.evaluate((viewport) => {
      ...
      elements.forEach(el => {
        ...
        if (style.width && style.width.includes('px')) {
          ...
          if (width > window.innerWidth && width > 300) { // Ignore very small fixed widths
            issues.push({
