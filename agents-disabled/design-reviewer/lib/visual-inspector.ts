...
import {
  SUMMIT_CHRONICLES_BRAND_CONFIG,
...
export class VisualInspector {
    ...
    const brokenImages = await this.page.evaluate(() => {
      ...
      return images
        ...
        .map((img) => ({
          ...
          selector:
            img.tagName +
            (img.className ? '.' + img.className.replace(/\s+/g, '.') : ''),
        }));
    });
    ...
    const colorUsage = await this.page.evaluate((brandColors) => {
      ...
      elements.forEach((el) => {
        ...
        if (
          color &&
          color !== 'rgba(0, 0, 0, 0)' &&
          !brandColors.some((bc: string) => color.includes(bc))
        ) {
    ...
    const layoutIssues = await this.page.evaluate(() => {
      ...
      if (
        ...
        issues.push({
          ...
          message:
            'Page has horizontal scroll which may indicate layout problems',
        });
      }
  ...
  async performFullVisualInspection(): Promise<VisualIssue[]> {
    const [brokenImages, colorIssues, fontIssues, layoutIssues] =
      await Promise.all([
