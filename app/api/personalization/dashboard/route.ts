...
async function generatePersonalizedDashboard(
  ...
  const totalHours =
    activities.reduce((sum, act) => sum + (act.moving_time || 0), 0) / 3600;
  const totalElevation = activities.reduce(
    (sum, act) => sum + (act.total_elevation_gain || 0),
    0
  );
  const avgDuration =
    activities.length > 0 ? totalHours / activities.length : 0;
...
function generateRecommendedContent(profile: UserProfile) {
  ...
  if (
    profile.primaryGoals.some((goal) => goal.toLowerCase().includes('everest'))
  ) {
    content.push({
      type: 'training',
  ...
  if (
    profile.primaryGoals.some((goal) =>
