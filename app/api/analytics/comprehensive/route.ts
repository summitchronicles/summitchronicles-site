...
function calculatePerformanceMetrics(activities: any[]): PerformanceMetrics {
  ...
  const totalDistance =
    activities.reduce((sum, act) => sum + (act.distance || 0), 0) / 1000; // Convert to km
  const totalElevation = activities.reduce(
    (sum, act) => sum + (act.total_elevation_gain || 0),
    0
  );
  const totalTime =
    activities.reduce((sum, act) => sum + (act.moving_time || 0), 0) / 3600; // Convert to hours
...
function calculateTrends(activities: any[], timeframe: string): TrendData[] {
  ...
  const trends: TrendData[] = [
    {
      ...
      current:
        (currentMetrics.totalDistance * currentMetrics.workoutFrequency) / 4, // Approximate weekly
      previous: 0,
      trend: 'stable',
      changePercent: 0,
      unit: 'km',
  ...
  trends.forEach((trend) => {
    trend.previous =
      Math.round(generatePreviousValue(trend.current, 0.15) * 10) / 10;
    const change = trend.current - trend.previous;
    trend.changePercent =
      trend.previous > 0
        ? Math.round((change / trend.previous) * 1000) / 10
        : 0;
...
function getTimeSpanInWeeks(activities: any[]): number {
  ...
  const dates = activities
    .map((act) => new Date(act.start_date || act.date))
    .sort();
