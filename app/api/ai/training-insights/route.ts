...
function parseTrainingInsights(
  aiResponse: string,
  ...
  for (const line of lines) {
    ...
    // Detect sections
    if (
      trimmed.toLowerCase().includes('progress assessment') ||
      trimmed.startsWith('1.')
    ) {
      currentSection = 'progress';
      continue;
    } else if (
      trimmed.toLowerCase().includes('areas for improvement') ||
      trimmed.startsWith('2.')
    ) {
      currentSection = 'improvement';
      continue;
    } else if (
      trimmed.toLowerCase().includes('specific recommendations') ||
      ...
      trimmed.toLowerCase().includes('next training cycle') ||
      trimmed.toLowerCase().includes('next week')
    ) {
    ...
    switch (currentSection) {
      ...
      case 'recommendations':
        if (
          ...
          specificRecommendations[specificRecommendations.length - 1] +=
            ' ' + trimmed;
        }
        break;
  ...
  if (!summary) {
    summary =
      aiResponse.split('\n')[0] ||
      'Analysis of your recent training activities shows consistent effort toward your mountaineering goals.';
  }

  if (!progressAssessment) {
    progressAssessment =
      'Your training shows good consistency with room for targeted improvements in specific areas.';
  }

  if (areasForImprovement.length === 0) {
    areasForImprovement.push(
      'Increase training volume gradually',
      'Focus on sport-specific endurance',
      'Incorporate more elevation gain'
    );
  }

  if (specificRecommendations.length === 0) {
    specificRecommendations.push(
      'Plan 1-2 longer endurance sessions per week',
      'Add weighted pack training',
      'Include technical skill practice'
    );
  }
  ...
  if (!nextWeekPlan) {
    nextWeekPlan =
      'Focus on building your aerobic base with longer, moderate-intensity sessions while maintaining technical skill practice.';
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const sampleInsights: TrainingInsights = {
      summary:
        "Your recent training shows excellent consistency with a good mix of endurance and technical activities. You're building a solid aerobic base while maintaining skill development.",
      progressAssessment:
        "Strong upward trend in training volume and intensity. Your endurance activities show steady improvement, and you're maintaining good consistency in technical training.",
      areasForImprovement: [
      ...
      nextWeekPlan:
        'Focus on one long endurance session with significant elevation gain, maintain 2-3 moderate cardio sessions, add one weighted pack training session, and include technical skill practice. Ensure proper recovery between high-intensity sessions.',
      confidence: 0.82,
      analysisDate: new Date().toISOString(),
    };
