...
export const POST = protectionPresets.adminEndpoint(async (request: ProtectedRequest) => {
  ...
  try {
    const { type = 'incremental', critical_only = false } = await request.json();
...
function calculateBackupHealth(backupLogs: any[]) {
  ...
  if (hoursSinceLastBackup > 26) healthScore -= 30; // More than 26 hours
  ...
  return {
    ...
    recommendations: generateRecommendations(hoursSinceLastBackup, successRate, avgDuration)
  };
}

function generateRecommendations(hoursSinceLastBackup: number, successRate: number, avgDuration: number): string[] {
  const recommendations: string[] = [];

  if (hoursSinceLastBackup > 26) {
    recommendations.push('⚠️ Last backup is overdue - check cron job configuration');
  }

  if (successRate < 80) {
    recommendations.push('🔧 Low backup success rate - investigate recent failures');
  }

  if (avgDuration > 60000) {
    recommendations.push('⏱️ Backup duration is high - consider optimizing or using incremental backups');
  }
