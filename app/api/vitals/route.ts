...
export async function GET(req: NextRequest) {
  try {
    ...
    const hoursBack = timeRange === '1h' ? 1 : timeRange === '7d' ? 168 : timeRange === '30d' ? 720 : 24
    ...
    const aggregated = Object.fromEntries(
      Array.from(vitalsMap.entries()).map(([metricName, values]) => {
        const nums = values.map(v => v.metric_value).filter(v => typeof v === 'number')
        const avg = nums.length > 0 ? nums.reduce((a, b) => a + b) / nums.length : 0
        const p75 = nums.length > 0 ? nums.sort((a, b) => a - b)[Math.floor(nums.length * 0.75)] || 0 : 0
        const p95 = nums.length > 0 ? nums.sort((a, b) => a - b)[Math.floor(nums.length * 0.95)] || 0 : 0
