...
import { getRecentActivities, getAthleteStats, getAthleteProfile, getMockStravaData } from '../../../../lib/integrations/strava'
    ...
    if (activities && activities.length > 0) {
      try {
        ...
        const trainingEntries = activities.slice(0, 5).map(activity => ({
          ...
          description: activity.description || `${activity.type} activity from Strava`,
          metrics: {
            distance: activity.distance ? Math.round(activity.distance * 0.000621371 * 10) / 10 : undefined, // meters to miles
            elevationGain: activity.total_elevation_gain ? Math.round(activity.total_elevation_gain * 3.28084) : undefined, // meters to feet
