...
const DefaultErrorFallback: React.FC<ErrorFallbackProps> = ({
  ...
  return (
    <div className={`${isPageLevel ? 'min-h-screen' : 'min-h-64'} flex items-center justify-center p-6`}>
      <div className="max-w-md w-full text-center">
        <div className={`mx-auto mb-4 ${isPageLevel ? 'w-16 h-16' : 'w-12 h-12'} text-red-500`}>
          <AlertTriangle className="w-full h-full" />
        </div>
        
        <h2 className={`${isPageLevel ? 'text-2xl' : 'text-lg'} font-semibold text-spa-charcoal mb-2`}>
...
const MinimalErrorFallback: React.FC<ErrorFallbackProps> = ({ resetError }) => (
  <div className="flex items-center justify-center p-4 text-center">
    <div>
      ...
      <p className="text-sm text-spa-charcoal/70 mb-3">Component failed to load</p>
      <button
        onClick={resetError}
...
const InlineErrorFallback: React.FC<ErrorFallbackProps> = ({ error, resetError }) => (
  <div className="bg-red-50 border border-red-200 rounded p-3">
    <div className="flex items-start gap-2">
      <AlertTriangle className="w-4 h-4 text-red-500 mt-0.5 flex-shrink-0" />
      <div className="flex-1 min-w-0">
        <p className="text-sm text-red-800 font-medium">Error loading component</p>
