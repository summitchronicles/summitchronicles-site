...
export function PhotoShowcase({ 
  ...
  if (photos.length === 0) {
    return (
      <div className={`bg-white/90 backdrop-blur-sm rounded-2xl p-8 border border-spa-stone/10 ${className}`}>
        <div className="text-center">
          ...
          <h3 className="text-xl font-medium text-spa-charcoal mb-2">{title}</h3>
          <p className="text-spa-charcoal/60 text-sm">
            Photo gallery coming soon. Real summit and training photos will be added here.
          </p>
        </div>
      </div>
    )
          ...
          >
            <div className="bg-white/90 backdrop-blur-sm rounded-xl overflow-hidden border border-spa-stone/10 shadow-sm hover:shadow-lg transition-all duration-300">
              {photo.placeholder ? (
                ...
                <div className="aspect-[4/3] bg-gradient-to-br from-spa-mist to-spa-cloud flex items-center justify-center" role="img" aria-label={`Placeholder for ${photo.achievement || 'Summit Photo'}`}>
                  <div className="text-center">
                    <Camera className="w-12 h-12 text-spa-charcoal/30 mx-auto mb-2" aria-hidden="true" />
                    <div className="text-sm text-spa-charcoal/60">
                      {photo.achievement || 'Summit Photo'}
                    </div>
                    <div className="text-xs text-spa-charcoal/40 mt-1">Coming Soon</div>
                  </div>
                </div>
              ) : (
                <div className="aspect-[4/3] bg-cover bg-center" style={{ backgroundImage: `url(${photo.src})` }} role="img" aria-label={photo.alt}>
                  <div className="w-full h-full bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end">
                    <div className="p-4 text-white">
                      <ExternalLink className="w-4 h-4" aria-label="View full size image" />
                    </div>
                  </div>
                </div>
              )}
              ...
              {photo.caption && (
                <div className="p-4">
                  <p className="text-sm font-medium text-spa-charcoal mb-1">{photo.caption}</p>
                  {photo.location && (
                    <p className="text-xs text-spa-charcoal/60">{photo.location}</p>
                  )}
                  {photo.date && (
                    <p className="text-xs text-spa-charcoal/60">{photo.date}</p>
                  )}
                </div>
              )}
            </div>
        ...
        <div className="text-center">
          <div className="bg-summit-gold/10 border border-summit-gold/20 rounded-lg p-4">
            <p className="text-sm text-spa-charcoal/80">
              📸 <strong>Photo Integration Ready:</strong> This component is prepared for your summit and training photos. 
              Simply replace the placeholder images with your actual expedition documentation.
            </p>
          </div>
        </div>
      )}
