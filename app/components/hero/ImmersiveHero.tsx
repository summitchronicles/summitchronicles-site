...
import { MountainScene, MountainSceneConfig } from "@/lib/three-scene";
...
const defaultSceneConfig: MountainSceneConfig = {
  peaks: 5,
  layers: 4,
  fog: true,
  particles: true,
...
export default function ImmersiveHero({ className = "" }: ImmersiveHeroProps) {
  const sceneRef = useRef<HTMLDivElement>(null);
  const mountainScene = useRef<MountainScene | null>(null);
  ...
  useEffect(() => {
    if (!sceneRef.current || mountainScene.current) return;

    try {
      mountainScene.current = new MountainScene(sceneRef.current, defaultSceneConfig);
      setSceneLoaded(true);
    } catch (error) {
    ...
    return () => {
      if (mountainScene.current) {
        mountainScene.current.dispose();
        mountainScene.current = null;
      }
    };
  }, []);
  ...
  return (
    <motion.section
      ...
      animate="visible"
      className={clsx(
        "mountain-scene relative min-h-screen overflow-hidden",
        "flex items-center justify-center",
        className
      )}
    >
      {/* Three.js Mountain Scene Background */}
      <div 
        ref={sceneRef}
        className="absolute inset-0 z-0"
        style={{ 
          background: 'linear-gradient(180deg, #0f172a 0%, #1e293b 50%, #374151 100%)'
        }}
      />

      {/* Fallback gradient if 3D scene fails */}
      {!sceneLoaded && (
        <div 
          className="absolute inset-0 z-0"
          style={{
            background: `
              ...
              linear-gradient(180deg, #0f172a 0%, #1e293b 50%, #374151 100%)
            `
          }}
        />
      )}
      ...
      {/* Main Hero Content */}
      <div className="relative z-content max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
        ...
        <motion.div variants={itemVariants} className="space-y-8 text-center lg:text-left">
          ...
          >
            <div className="flex items-center space-x-2 text-gray-300">
              <div className="w-2 h-2 bg-successGreen rounded-full animate-pulse" />
          ...
          >
            <GlassCard className="p-4 cursor-pointer hover:scale-105 transition-transform">
              <div className="text-center space-y-2">
                ...
                <div className="text-sm font-medium text-snowWhite">Analytics</div>
              </div>
            </GlassCard>
            ...
            <GlassCard className="p-4 cursor-pointer hover:scale-105 transition-transform">
              <div className="text-center space-y-2">
                <div className="text-2xl">🎯</div>
                <div className="text-sm font-medium text-snowWhite">Routes</div>
              </div>
            </GlassCard>
          </motion.div>
        </motion.div>
      ...
      <motion.div
        variants={itemVariants}
        className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-content"
      >
        <motion.div
          animate={{ y: [0, 10, 0] }}
