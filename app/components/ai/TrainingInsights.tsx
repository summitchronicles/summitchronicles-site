...
export function TrainingInsights({
  ...
  const getProgressIcon = (assessment: string) => {
    if (
      assessment.toLowerCase().includes('excellent') ||
      assessment.toLowerCase().includes('strong')
    ) {
      return <CheckCircle className="w-5 h-5 text-green-600" />;
    }
    if (
      assessment.toLowerCase().includes('good') ||
      assessment.toLowerCase().includes('steady')
    ) {
      return <TrendingUp className="w-5 h-5 text-blue-600" />;
    }
    if (
      assessment.toLowerCase().includes('concern') ||
      assessment.toLowerCase().includes('risk')
    ) {
        ...
        {error ? (
          ...
          <div className="space-y-6">
            ...
            <div className="text-xs text-spa-charcoal/50 text-center">
              ...
              {new Date(insights.analysisDate).toLocaleDateString()} • Based on{' '}
          ...
          <div className="text-center py-8">
            ...
            <p className="text-spa-charcoal/60 mb-4">
              Generate AI-powered training insights based on your activity data
            </p>
            <button
              onClick={generateInsights}
