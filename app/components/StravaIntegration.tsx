    ...
    <div className="bg-white/90 backdrop-blur-sm rounded-xl p-6 border border-spa-stone/10 shadow-sm hover:shadow-md transition-all group">
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <div
              className={cn('p-2 rounded-lg', getActivityColor(activity.type))}
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="text-center p-3 bg-spa-mist/10 rounded-lg">
          <div className="text-xl font-light text-spa-charcoal">
...
export function StravaActivityFeed() {
  ...
  const fetchActivities = async () => {
    try {
      ...
      console.log(
        `📊 Loaded ${data.activities?.length || 0} activities from ${data.source || 'unknown'}`
      );
    } catch (err) {
      setError(
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-spa-charcoal/70">
            Real-time data synchronized from Strava
          </p>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="bg-white/90 backdrop-blur-sm rounded-xl p-6 border border-spa-stone/10 animate-pulse"
            >
              ...
              <div className="grid grid-cols-4 gap-4 mb-4">
                {Array.from({ length: 4 }).map((_, j) => (
                  <div
                    key={j}
                    className="text-center p-3 bg-spa-mist/10 rounded-lg"
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {(activities.length > 0 ? activities : fallbackActivities).map(
            (activity, index) => {
              const activityData = {
                ...
                elevation:
                  activity.elevation || activity.total_elevation_gain || 0,
              ...
              return (
                <ActivityCard key={activityData.id} activity={activityData} />
              );
            }
          )}
        </div>
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-spa-charcoal/70 text-sm">
              Data-driven analysis of training progression
            </p>
          </div>
        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-spa-charcoal/80 leading-relaxed">
                Your elevation gains are impressive this week! Consider adding
                more recovery time between high-intensity sessions to optimize
                performance gains and maintain systematic progression.
              </p>
            </div>
          </div>
        </div>
