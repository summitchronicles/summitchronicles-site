...
export function PersonalizedDashboard({ 
  ...
  const defaultProfile: UserProfile = {
    name: 'Sunith Kumar',
    level: 'intermediate',
    primaryGoals: ['Mount Everest expedition preparation', 'High-altitude endurance', 'Technical climbing skills'],
  ...
  const generateSampleContent = (): PersonalizedContent => {
    ...
    return {
      ...
      priorityActions: [
        {
          ...
          description: 'Aim for 6-8 hours with 1200m+ elevation gain to build your aerobic base',
          urgency: 'high'
        },
        {
          type: 'planning',
          title: 'Update Expedition Gear List',
          description: 'Review and finalize equipment list for your upcoming expedition',
          urgency: 'medium'
        },
        {
          type: 'safety',
          title: 'Practice Avalanche Rescue Scenarios',
          description: 'Maintain proficiency with beacon, probe, and shovel techniques',
          urgency: 'medium'
        }
      ],
      recommendedContent: [
      ...
      weeklyFocus: {
        ...
        description: 'Focus on building your cardiovascular foundation with longer, moderate-intensity sessions',
        goals: [
    ...
    >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-blue-100 text-sm">{personalizedContent.welcomeMessage}</p>
            </div>
          </div>
          <div className="text-right">
            <div className="text-sm opacity-90">Training Level</div>
            <div className="text-lg font-medium capitalize">{profile.level}</div>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 space-y-6">
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-spa-charcoal mb-1">{action.title}</h4>
                    <p className="text-sm text-spa-charcoal/70">{action.description}</p>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <div className="text-2xl font-light text-alpine-blue">
                  {personalizedContent.progressMetrics.currentWeek.activitiesCompleted}
                </div>
                <div className="text-xs text-spa-charcoal/60">Activities</div>
              </div>
              <div>
                <div className="text-2xl font-light text-alpine-blue">
                  {Math.round(personalizedContent.progressMetrics.currentWeek.totalHours)}h
                </div>
                <div className="text-xs text-spa-charcoal/60">Training Time</div>
              </div>
              <div>
                <div className="text-2xl font-light text-alpine-blue">
                  {Math.round(personalizedContent.progressMetrics.currentWeek.elevationGained)}m
                </div>
          ...
          <div className="bg-spa-cloud/10 rounded-lg p-4">
            ...
            <div className="space-y-2">
              {Object.entries(personalizedContent.progressMetrics.trends).map(([metric, trend]) => (
                <div key={metric} className="flex items-center justify-between">
                  <span className="text-sm text-spa-charcoal capitalize">{metric}</span>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <p className="text-sm text-blue-800 mb-3">{personalizedContent.weeklyFocus.description}</p>
            ...
            {personalizedContent.recommendedContent.map((content, index) => (
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <h4 className="font-medium text-spa-charcoal mb-1">{content.title}</h4>
                    <div className="flex items-center gap-3 text-sm text-spa-charcoal/60">
                      <span className="px-2 py-1 bg-gray-100 rounded text-xs">{content.category}</span>
