...
export async function ingestContentToKnowledgeBase(): Promise<ContentIngestionResult> {
  ...
  try {
    ...
    const [trainingEntries, blogPosts, expeditionUpdates, personalStories] = await Promise.all([
    ...
    for (const entry of trainingEntries) {
      try {
        ...
        result.categories[document.category] = (result.categories[document.category] || 0) + 1
      } catch (error) {
    ...
    console.log(`Content ingestion completed: ${result.documentsAdded}/${result.documentsProcessed} documents added`)
    return result
...
function transformTrainingEntry(entry: any): Omit<KnowledgeDocument, 'id' | 'embedding' | 'created_at' | 'updated_at'> {
  const content = [
    entry.description || '',
    entry.metrics ? `Metrics: Distance ${entry.metrics.distance || 'N/A'}, Elevation ${entry.metrics.elevationGain || 'N/A'}, Duration ${entry.duration || 'N/A'}min` : '',
    entry.location ? `Location: ${entry.location.name}, Weather: ${entry.location.weather}` : ''
  ].filter(Boolean).join('\n\n')

  return {
    title: entry.title,
    content: content || `${entry.type} training session completed on ${entry.date}`,
...
function transformBlogPost(post: any): Omit<KnowledgeDocument, 'id' | 'embedding' | 'created_at' | 'updated_at'> {
  return {
    title: post.title,
...
function transformExpeditionUpdate(update: any): Omit<KnowledgeDocument, 'id' | 'embedding' | 'created_at' | 'updated_at'> {
  const content = [
    update.content || '',
...
function transformPersonalStory(story: any): Omit<KnowledgeDocument, 'id' | 'embedding' | 'created_at' | 'updated_at'> {
  return {
    title: story.title,
...
function mapIntensityToDifficulty(intensity: string): 'beginner' | 'intermediate' | 'advanced' | 'expert' {
  switch (intensity?.toLowerCase()) {
    case 'low': return 'beginner'
    case 'moderate': return 'intermediate'
    case 'high': return 'advanced'
    case 'maximum': return 'expert'
    default: return 'intermediate'
  }
}

function mapTrainingTypeToMountainType(type: string): 'technical' | 'endurance' | 'mixed' | 'expedition' {
  switch (type?.toLowerCase()) {
    case 'climbing':
    case 'technical': return 'technical'
    case 'cardio':
    case 'hiking': return 'endurance'
    case 'strength': return 'mixed'
    case 'recovery': return 'mixed'
...
function extractEquipmentFromTags(tags: string[]): string[] {
  const equipmentKeywords = [
    'ice-axes', 'crampons', 'harness', 'helmet', 'rope', 'carabiners',
    'ice-screws', 'pitons', 'nuts', 'cams', 'belay-device', 'avalanche-beacon'
  ]
  return tags.filter(tag => equipmentKeywords.some(keyword => tag.toLowerCase().includes(keyword)))
}

function extractDifficultyFromContent(content: string): 'beginner' | 'intermediate' | 'advanced' | 'expert' {
  ...
  if (text.includes('expert') || text.includes('professional') || text.includes('extreme')) return 'expert'
  if (text.includes('advanced') || text.includes('technical') || text.includes('complex')) return 'advanced'
  return 'intermediate'
}

function extractMountainTypeFromTags(tags: string[]): 'technical' | 'endurance' | 'mixed' | 'expedition' {
  const technicalTags = ['technical', 'climbing', 'ice', 'rock', 'mixed']
  const expeditionTags = ['expedition', 'high-altitude', 'everest', 'denali']
  
  if (tags.some(tag => expeditionTags.some(keyword => tag.toLowerCase().includes(keyword)))) {
    return 'expedition'
  }
  if (tags.some(tag => technicalTags.some(keyword => tag.toLowerCase().includes(keyword)))) {
    return 'technical'
  }
  return 'endurance'
}

function extractMountainTypeFromLocation(location: string): 'technical' | 'endurance' | 'mixed' | 'expedition' {
  if (!location) return 'mixed'
  
  const loc = location.toLowerCase()
  if (loc.includes('everest') || loc.includes('denali') || loc.includes('8000')) return 'expedition'
  if (loc.includes('technical') || loc.includes('ice') || loc.includes('rock')) return 'technical'
  return 'mixed'
}
...
export async function scheduleContentIngestion(): Promise<void> {
  try {
    ...
    if (result.success) {
      console.log(`Content ingestion successful: ${result.documentsAdded} documents added`)
    } else {
