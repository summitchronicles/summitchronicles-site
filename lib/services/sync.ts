...
class DataSyncService {
  ...
  start() {
    ...
    // Initial sync
    this.performSync()
    console.log(`Data sync service started with ${this.config.intervalMinutes}min intervals`)
  }
  ...
  private async syncStravaData() {
    try {
      ...
      const trainingEntries = data.data.activities.slice(0, 5).map((activity: any) => ({
        _type: 'trainingEntry',
        ...
        description: activity.description || `${activity.type} activity synced from Strava`,
        metrics: {
          distance: activity.distance ? Math.round(activity.distance * 0.000621371 * 10) / 10 : undefined,
          elevationGain: activity.total_elevation_gain ? Math.round(activity.total_elevation_gain * 3.28084) : undefined,
  ...
  private async syncWeatherData() {
    try {
      ...
      const weatherSummary = {
        ...
        locations: weatherResults.filter(r => r.data).map(r => ({
          name: r.location,
  ...
  private async syncAIKnowledgeBase() {
    try {
      ...
      this.setCache('ai-last-sync', new Date().toISOString(), 24 * 60 * 60 * 1000) // 24hr TTL
  ...
  private calculateIntensity(activity: any): string {
    ...
    if (activity.type === 'WeightTraining' || activity.type === 'Crossfit') return 'high'
