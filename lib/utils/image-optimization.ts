...
export const generateBlurDataURL = (width: number = 400, height: number = 300): string => {
  const svg = `
...
export const generateResponsiveSizes = (
  ...
  const sizeQueries = entries.map(([key, size]) => {
    const breakpoint = RESPONSIVE_BREAKPOINTS[key as keyof typeof RESPONSIVE_BREAKPOINTS]
