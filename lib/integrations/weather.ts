...
export async function getWeatherData(location: keyof typeof mountainLocations): Promise<WeatherData | null> {
  try {
    ...
    return {
      ...
      forecast: data.daily.slice(0, 7).map((day: any): WeatherForecast => ({
      ...
      alerts: data.alerts?.map((alert: any): WeatherAlert => ({
...
export function calculateHighAltitudeConditions(weather: WeatherData): HighAltitudeConditions {
  ...
  // Assess visibility
  const visibilityRating = assessVisibility(current.visibility, current.weather.main)
...
function calculateOxygenLevel(elevation: number): number {
  ...
  const pressureAtAltitude = seaLevelPressure * Math.pow(1 - 0.0065 * elevation / 288.15, 5.255)
  return Math.round((pressureAtAltitude / seaLevelPressure) * 100)
}

function assessAvalancheRisk(weather: WeatherData): HighAltitudeConditions['avalancheRisk'] {
  const { current, forecast } = weather
  let riskScore = 0
  
  // Temperature factors
  if (current.temperature > -5 && current.temperature < 2) riskScore += 2 // Warming conditions
  if (forecast.some(day => day.temperature.max - day.temperature.min > 10)) riskScore += 1 // Large temp swings
  ...
  if (forecast.some(day => day.precipitation.probability > 50)) riskScore += 1
  if (forecast.some(day => day.precipitation.snow && day.precipitation.snow > 10)) riskScore += 2
...
function assessIcefallConditions(weather: WeatherData): HighAltitudeConditions['icefallConditions'] {
...
function assessVisibility(visibility: number, weatherMain: string): HighAltitudeConditions['visibilityRating'] {
  if (weatherMain.includes('fog') || weatherMain.includes('mist')) return 'poor'
...
function assessClimbingWindow(weather: WeatherData): HighAltitudeConditions['climbingWindow'] {
  const { current, forecast } = weather
  ...
  const isCurrentlyGood = current.wind_speed < 15 && !current.weather.main.includes('Thunderstorm')
