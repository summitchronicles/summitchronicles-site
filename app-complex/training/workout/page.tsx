...
export default function WorkoutPage() {
  ...
  const fetchWorkout = async () => {
    ...
    try {
      ...
      if (data.workout && data.workout.exercises.some((ex: Exercise) => ex.actual_sets?.length > 0)) {
  ...
  const getTotalVolume = (exercise: Exercise): number => {
    return exercise.actual_sets?.reduce((total, set) => {
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        {!workout ? (
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-white mb-4">No Workout Scheduled</h2>
            <p className="text-white/60 mb-8">
              No strength training session found for {new Date(currentDate).toLocaleDateString()}.
            </p>
            <Link
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h1 className="text-2xl font-bold text-white">{workout.session_type}</h1>
                  <p className="text-white/60">
                    {workout.day_name} • {new Date(workout.date).toLocaleDateString()}
                  </p>
                </div>
                ...
                {workoutStarted && startTime && (
                  <div className="text-right">
                    <div className="flex items-center gap-2 text-summitGold">
                      ...
                      <span className="font-mono text-lg">{formatElapsedTime()}</span>
                    </div>
            ...
            <div className="space-y-6 mb-8">
              {workout.exercises.map((exercise, index) => (
                <ExerciseCard
                  ...
                  onCompleteExercise={(notes) => completeExercise(exercise.id, notes)}
                />
              ))}
            </div>
...
function ExerciseCard({ 
  ...
  const totalVolume = exercise.actual_sets?.reduce((total, set) => {
    return total + ((set.weight_used || 0) * (set.reps_completed || 0));
  }, 0) || 0;
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              `}>
                {exercise.completed ? <CheckCircleIcon className="w-4 h-4" /> : exercise.sequence}
              </div>
              <h3 className="text-lg font-semibold text-white">{exercise.name}</h3>
            </div>
            
            <div className="flex items-center gap-4 text-sm text-white/60">
              <span>{exercise.planned_sets} sets × {exercise.planned_reps}</span>
      ...
      {expanded && (
        ...
        >
                ...
                {exercise.actual_sets.map((set, setIndex) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      <span className="text-white">{set.reps_completed} reps</span>
                      <span className="text-summitGold">{set.weight_used}kg</span>
                      <span className="text-white/60">RPE {set.actual_rpe}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {workoutStarted && !exercise.completed && (
            <div className="mb-6">
              <h4 className="text-white font-semibold mb-3">Log Set {newSet.set_number}</h4>
              <div className="grid grid-cols-3 gap-4 mb-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewSet({...newSet, reps_completed: parseInt(e.target.value) || undefined})}
                    className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-center"
                    placeholder="0"
                  />
                </div>
                <div>
                  <label className="block text-white/60 text-xs mb-1">Weight (kg)</label>
                  <input
                    type="number"
                    step="0.5"
                    value={newSet.weight_used || ''}
                    onChange={(e) => setNewSet({...newSet, weight_used: parseFloat(e.target.value) || undefined})}
                    className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-center"
                    placeholder="0.0"
                  />
                </div>
                <div>
                  <label className="block text-white/60 text-xs mb-1">RPE</label>
                  <input
                    type="number"
                    min="1"
                    max="10"
                    ...
                    onChange={(e) => setNewSet({...newSet, actual_rpe: parseFloat(e.target.value) || undefined})}
                    className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-center"
                    placeholder="1-10"
                  />
                </div>
              ...
              <button
                ...
                disabled={!newSet.reps_completed || !newSet.weight_used || !newSet.actual_rpe}
          ...
          {exercise.completed && exercise.notes && (
            <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3">
              <p className="text-green-400 text-sm font-medium mb-1">Exercise Notes:</p>
