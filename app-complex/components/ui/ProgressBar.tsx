...
interface ProgressBarProps {
  ...
  label?: string;
  showValue?: boolean;
  variant?: 'default' | 'gradient' | 'altitude' | 'success' | 'warning' | 'danger';
  size?: 'sm' | 'md' | 'lg';
  animated?: boolean;
  glow?: boolean;
  className?: string;
...
const variantConfig = {
  ...
  altitude: {
    background: 'var(--color-light-gray)',
    foreground: 'linear-gradient(90deg, var(--altitude-base) 0%, var(--altitude-summit) 100%)',
    ...
    <div className={clsx("space-y-2", className)}>
      ...
      {(label || showValue) && (
        <div className="flex justify-between items-center">
          {label && (
            <span className={clsx("font-medium text-charcoal", sizeStyle.textSize)}>
              {label}
            </span>
          )}
          {showValue && (
            <motion.span 
              className={clsx("font-semibold text-stoneGray", sizeStyle.textSize)}
