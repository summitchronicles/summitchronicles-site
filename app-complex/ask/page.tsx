...
export default function AskPage() {
  ...
  const getQuestionCategory = (question: string): string => {
    ...
    if (
      ...
      return 'training';
    if (
      lowerQuestion.includes('gear') ||
      lowerQuestion.includes('equipment') ||
      lowerQuestion.includes('boot')
    )
      return 'gear';
    if (
      lowerQuestion.includes('everest') ||
      lowerQuestion.includes('k2') ||
      lowerQuestion.includes('summit') ||
      lowerQuestion.includes('expedition')
    )
      return 'expeditions';
    if (
      lowerQuestion.includes('altitude') ||
      lowerQuestion.includes('acclimatization') ||
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
          ...
          >
            ...
            <p className="text-xl text-white/60 max-w-2xl mx-auto leading-relaxed mb-12">
              Get instant answers about expeditions, training techniques, gear
              recommendations, and mountaineering wisdom powered by AI.
            </p>
          ...
          <div className="max-w-4xl mx-auto px-6">
            ...
            {showFilters && hasActiveFilters && (
              <div className="mb-6 text-white/60 text-sm">
                Showing {filteredHistory.length} of {conversationHistory.length}{' '}
                conversations
              </div>
            )}

            <div className="space-y-6">
              {(hasActiveFilters ? filteredHistory : conversationHistory)
                .slice(0, 10)
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <div
                            className={`px-2 py-1 rounded-full text-xs font-medium ${
                              conversation.category === 'training'
                                ? 'bg-blue-500/20 text-blue-400'
                                : conversation.category === 'gear'
                                  ? 'bg-green-500/20 text-green-400'
                                  : conversation.category === 'expeditions'
                                    ? 'bg-purple-500/20 text-purple-400'
                                    : conversation.category === 'health'
                                      ? 'bg-red-500/20 text-red-400'
                                      : 'bg-gray-500/20 text-gray-400'
                            }`}
                          >
                            {categories.find(
                              (c) => c.id === conversation.category
                            )?.label || 'General'}
                          </div>
                        </div>
                        <div className="text-xs text-white/50">
