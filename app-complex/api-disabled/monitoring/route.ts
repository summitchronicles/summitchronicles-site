...
const MonitoringQuerySchema = z.object({
  metric: z
    .enum(['uptime', 'performance', 'errors', 'overview'])
    .optional()
    .default('overview'),
...
async function getUptimeMetrics(
  timeRange: string
): Promise<MonitoringMetrics['uptime']> {
...
async function getPerformanceMetrics(
  timeRange: string
): Promise<MonitoringMetrics['performance']> {
  ...
  try {
    ...
    const responseTimes = aiData?.map((d) => d.response_time) || [
      1200, 1800, 2100, 1500, 1900,
    ];
...
async function getErrorMetrics(
  timeRange: string
): Promise<MonitoringMetrics['errors']> {
  const supabase = getSupabaseAdmin();
  ...
  try {
    ...
    const errorsByTypeArray = Object.entries(errorsByType).map(
...
async function getOverviewMetrics(): Promise<MonitoringMetrics['overview']> {
  ...
  return {
    ...
    deploymentInfo: {
      ...
      deployedAt:
        process.env.VERCEL_DEPLOYMENT_DATE || new Date().toISOString(),
