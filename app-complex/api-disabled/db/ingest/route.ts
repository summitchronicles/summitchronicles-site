...
export async function POST(req: NextRequest) {
  ...
  if (!envSecret)
    return bad('Server not configured: missing INGEST_SECRET', 500);
  ...
  if (insDocErr || !docRow) {
    return bad(
      `Insert document failed: ${insDocErr?.message || 'unknown'}`,
      500
    );
  }
  ...
  try {
    const perChunk = await Promise.all(
      chunks.map(async (c, i) => {
        ...
        if (!Array.isArray(v))
          throw new Error(`embedText returned non-array at chunk ${i}`);
        return v as number[];
      })
    );
    vectors = perChunk;
  ...
  if (insChunksErr)
    return bad(`Insert chunks failed: ${insChunksErr.message}`, 500);
