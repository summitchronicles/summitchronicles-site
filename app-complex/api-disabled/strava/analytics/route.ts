...
async function fetchExistingStravaData() {
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/strava/recent`
  );
...
function aggregateMonthlyData(activities: StravaActivity[]): {
  ...
  const monthsWithData = Array.from(monthlyStats.keys()).sort(
    (a, b) => allMonths.indexOf(a) - allMonths.indexOf(b)
  );

  const running = monthsWithData.map((month) => {
    const data = monthlyStats.get(month)?.running || {
      distance: 0,
      time: 0,
      sessions: 0,
      speeds: [],
  ...
  const hiking = monthsWithData.map((month) => {
    const data = monthlyStats.get(month)?.hiking || {
      distance: 0,
      time: 0,
      sessions: 0,
      elevation: 0,
  ...
  const strength = monthsWithData.map((month) => {
    const data = monthlyStats.get(month)?.strength || {
      sessions: 0,
      duration: 0,
    };
    ...
    const totalWeight =
      data.sessions > 0 ? (baseWeight + progression) * data.sessions : 0;

    return {
      month,
      distance: 0, // Not applicable for strength training
      hours:
        data.sessions > 0 ? Math.round(data.duration / data.sessions / 60) : 0,
      totalWeight,
      sessions: data.sessions,
      avgDuration:
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(
      `Processing ${allActivities.length} activities from existing Strava connection`
    );
    ...
    const totalStats = {
      running: {
        totalDistance: monthlyData.running.reduce(
          (sum, month) => sum + month.distance,
          0
        ),
        totalHours: monthlyData.running.reduce(
          (sum, month) => sum + month.hours,
          0
        ),
        totalSessions: monthlyData.running.reduce(
          (sum, month) => sum + month.sessions,
          0
        ),
      },
      hiking: {
        totalDistance: monthlyData.hiking.reduce(
          (sum, month) => sum + month.distance,
          0
        ),
        totalHours: monthlyData.hiking.reduce(
          (sum, month) => sum + month.hours,
          0
        ),
        totalSessions: monthlyData.hiking.reduce(
          (sum, month) => sum + month.sessions,
          0
        ),
        totalElevation: monthlyData.hiking.reduce(
          (sum, month) => sum + (month.elevation || 0),
          0
        ),
      },
      strength: {
        totalSessions: monthlyData.strength.reduce(
          (sum, month) => sum + month.sessions,
          0
        ),
        totalWeight: monthlyData.strength.reduce(
          (sum, month) => sum + (month.totalWeight || 0),
          0
        ),
        totalHours: monthlyData.strength.reduce(
          (sum, month) =>
            sum + (month.sessions * (month.avgDuration || 0)) / 60,
          0
        ),
      },
    };
