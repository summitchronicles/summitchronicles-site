...
export async function GET() {
  try {
    ...
    const stats = {
      ...
      total_distance: transformedActivities
        .reduce((sum: number, a: any) => sum + (parseFloat(a.distance) || 0), 0)
        .toFixed(1),
      total_duration: transformedActivities.reduce(
        (sum: number, a: any) => sum + (a.duration || 0),
        0
      ),
      total_elevation: transformedActivities.reduce(
        (sum: number, a: any) => sum + (a.elevation_gain || 0),
        0
      ),
