...
export const GET = protectionPresets.adminEndpoint(async (request: ProtectedRequest) => {
  try {
    ...
    switch (action) {
      case 'stats':
        const stats = await MultiUserDatabase.getTrainerDashboardStats(trainerId);
        return NextResponse.json({ stats });

      case 'clients':
        const clients = await MultiUserDatabase.getClientProgressSummaries(trainerId);
      ...
      case 'relationships':
        const relationships = await MultiUserDatabase.getTrainerClients(trainerId);
        return NextResponse.json({ relationships });

      case 'overview':
        // Combined overview data
        const [overviewStats, overviewClients, overviewPrograms] = await Promise.all([
      ...
      default:
        return NextResponse.json({ 
          error: 'Invalid action. Use: stats, clients, programs, relationships, or overview' 
        }, { status: 400 });
    }
  ...
  try {
    ...
    switch (action) {
      case 'create_client_relationship':
        ...
        const relationship = await MultiUserDatabase.createTrainerClientRelationship({
      ...
      case 'update_client_access':
        ...
        const updatedRelationship = await MultiUserDatabase.updateRelationshipStatus(
          relationship_id, 
      ...
      case 'assign_program':
        const { client_id, program_id, start_date, target_end_date, custom_modifications } = body;
