    ...
    try {
      ...
      switch (action) {
        case 'insights':
          ...
          const trainingData = await TrainingRAG.extractTrainingData(
            startDate,
            endDate
          );
          const insights =
            await TrainingRAG.generateTrainingInsights(trainingData);
        ...
        case 'embeddings':
          ...
          const data = await TrainingRAG.extractTrainingData(
            startDate,
            endDate
          );
      ...
      switch (action) {
        case 'update_knowledge_base':
          ...
          const startDate = new Date(
            Date.now() - (dateRange || 30) * 24 * 60 * 60 * 1000
          )
            .toISOString()
            .split('T')[0];
