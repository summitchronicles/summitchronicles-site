  ...
  try {
    ...
    switch (action) {
      case 'sync':
        ...
        const activities = await EnhancedStravaIntegration.syncEnhancedActivities(limit);
      ...
      case 'insights':
        ...
        const insights = await EnhancedStravaIntegration.getTrainingInsights(startDate, endDate);
      ...
      case 'duplicates':
        ...
        const duplicates = await EnhancedStravaIntegration.detectDuplicateActivities(date);
...
export const POST = protectionPresets.adminEndpoint(async (request: ProtectedRequest) => {
  try {
    ...
    return NextResponse.json({ 
      error: error.message || 'Failed to process enhanced Strava POST request' 
    }, { status: 500 });
  }
});