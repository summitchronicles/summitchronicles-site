      ...
      switch (action) {
        ...
        case 'assigned_programs':
          if (userRole === 'trainer') {
            ...
            const assignments =
              await MultiUserDatabase.getClientProgramAssignments({
                assigned_by: userId,
              });
        ...
        case 'program_details':
          ...
          const program = await MultiUserDatabase.getTrainingPrograms({
            id: programId,
          });
          if (!program.length) {
            return NextResponse.json(
        ...
        case 'summit_specific':
          ...
          if (!targetSummit) {
            return NextResponse.json(
              { error: 'summit parameter is required' },
              { status: 400 }
            );
          }
        ...
        case 'recommended':
          ...
          const recommendedPrograms =
            await MultiUserDatabase.getTrainingPrograms(filters);
        ...
        default:
          return NextResponse.json(
            {
              error:
                'Invalid action. Use: my_programs, public_templates, assigned_programs, program_details, summit_specific, or recommended',
            },
            { status: 400 }
          );
      }
...
export const POST = protectionPresets.apiEndpoint(
  async (request: ProtectedRequest) => {
    try {
      ...
      switch (action) {
        case 'create_program':
          ...
          const createdProgram =
            await MultiUserDatabase.createTrainingProgram(programData);
        ...
        case 'clone_template':
          ...
          const template = await MultiUserDatabase.getTrainingPrograms({
            id: template_id,
          });
        ...
        case 'update_program':
          ...
          const existingProgram = await MultiUserDatabase.getTrainingPrograms({
            id: program_id,
          });
          ...
          const canEdit =
            existingProgram[0].created_by === userId || userRole === 'admin';
          if (!canEdit) {
            return NextResponse.json(
              { error: 'Access denied' },
              { status: 403 }
          ...
          const updatedProgram = await MultiUserDatabase.updateTrainingProgram(
            program_id,
            updates
          );
        ...
        case 'assign_to_client':
          if (userRole !== 'trainer' && userRole !== 'admin') {
            return NextResponse.json(
              { error: 'Only trainers can assign programs' },
              { status: 403 }
            );
          }
          ...
          const relationships =
            await MultiUserDatabase.getTrainerClients(userId);
          const hasRelationship = relationships.some(
            (rel) => rel.client_id === client_id && rel.status === 'active'
          );
        ...
        case 'update_assignment':
          ...
          const canUpdate =
            assignmentData.assigned_by === userId ||
          ...
          const updatedAssignment =
            await MultiUserDatabase.updateProgramAssignment(
              assignment_id,
              assignment_updates
            );
        ...
        case 'publish_template':
          ...
          const publishedProgram =
            await MultiUserDatabase.updateTrainingProgram(publishProgramId, {
              is_public: true,
              is_template: true,
            });
