...
export const GET = protectionPresets.apiEndpoint(
  async (request: ProtectedRequest) => {
    try {
      ...
      switch (action) {
        case 'list':
          const plans = await MultiUserDatabase.getPeriodizationPlans({
            user_id: userId,
          });
        ...
        case 'current':
          ...
          if (currentPlan) {
            ...
            const loadMetrics = await analytics.calculateCurrentLoadMetrics(
              userId,
              currentPlan.id
            );
        ...
        case 'analytics':
          ...
          const analytics = new AdvancedPeriodization();
          const [peakPrediction, loadProgression, riskAssessment] =
            await Promise.all([
        ...
        case 'recommendations':
          ...
          const recommendations =
            await recommendationAnalytics.generateTrainingRecommendations(
              userId
            );
        ...
        default:
          return NextResponse.json(
            {
              error:
                'Invalid action. Use: list, current, analytics, or recommendations',
            },
            { status: 400 }
          );
      }
    ...
    try {
      ...
      switch (action) {
        case 'create_plan':
          ...
          const createdPlan =
            await MultiUserDatabase.createPeriodizationPlan(optimizedPlan);
        ...
        case 'update_plan':
          ...
          const updatedPlan = await MultiUserDatabase.updatePeriodizationPlan(
            plan_id,
            updates
          );
        ...
        case 'optimize_current':
          ...
          const reOptimizedPlan = await analytics.reOptimizePlan(
            userId,
            currentPlan.id
          );
        ...
        case 'record_performance':
          ...
          const shouldReOptimize = await analytics.shouldTriggerReOptimization(
            userId,
            benchmark
          );
