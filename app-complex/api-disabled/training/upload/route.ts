...
export const POST = protectionPresets.adminEndpoint(async (request: ProtectedRequest) => {
  try {
    ...
    if (!parsedPlan.title) {
      return NextResponse.json({ error: 'Could not parse training plan title' }, { status: 400 });
    }

    // Save to database
    const planId = await TrainingDatabase.saveTrainingPlan(parsedPlan, file.name, startDate);
